...
* A deployment of a `admission-controller`: this controller monitors the
...
[subs="+attributes",console]
----
kubectl create namespace suse-sec-adm-controller
----
...
[subs="+attributes",console]
----
helm install suse-sec-adm-controller \
...
The `admission-controller` manages a {admc-short-name} `PolicyServer`. You can
...
spec:
  image: dp.apps.rancher.io/containers/kubewarden-policy-server:1.36.0-9.1
  replicas: 2
  serviceAccountName: ~
  env:
...
| The name of the `ServiceAccount` to use for the `PolicyServer` deployment. If
  there is no provided value, the default `ServiceAccount` from the installation namespace, of `admission-controller`, gets used.
...
[subs="+attributes",console]
----
helm uninstall --namespace suse-sec-adm-controller suse-sec-adm-controller
----

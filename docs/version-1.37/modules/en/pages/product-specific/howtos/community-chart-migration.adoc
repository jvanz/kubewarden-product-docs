...
= Migration from the community single chart
...
:sidebar_label: Migration from the community single chart

== Migration from the {community-project-name} single chart to the {admc-name} chart
...
reshaping. The work is a cross-vendor registry and pull-secret convention
change, plus adopting the surviving CRDs into the new Helm release. Because
both charts keep CRDs and preserve user policies on uninstall, this is largely
an in-place migration performed in the same namespace.
...
[IMPORTANT]
====
The {admc-name} chart does not read `global.cattle.systemDefaultRegistry` or a
top-level `imagePullSecrets` list. Those keys have no effect and must not be
...
Back up all your {policy-server} instances and policy custom resources as a
safety net. Because the community chart preserves user CRs on uninstall, this
backup is a precaution rather than the primary recovery mechanism.
...
empty, which is fine — the {admc-name} chart defaults cover all required
settings.
...
policy-server) from `dp.apps.rancher.io`, which is a private registry. Create
an image pull secret in `$NAMESPACE` so the cluster can authenticate when
pulling these images:
...
[NOTE]
====
The community chart keeps its CRDs (`helm.sh/resource-policy: keep`) and its
cleanup hook preserves user-defined PolicyServers and policies, removing only
chart-managed defaults and their backing resources (Deployments, webhooks,
Services, Secrets). After uninstall, the CRDs and your user CRs remain in the
cluster for the {admc-name} chart to adopt. There is still a short enforcement
gap until the {admc-name} chart is ready.
====
...
present in the cluster. You only need to update any PolicyServer whose
`spec.image` was pinned to the community image. The following command patches
...
| `global.imagePullSecrets`
| Move under `global`; the `global.cattle.*` block is ignored by the {admc-name} chart
...
The {admc-name} chart reads `global.imagePullSecrets` instead. The Step 3 `yq`
command moves the list automatically.
...
release-name annotation changes — the namespace annotation stays consistent.
...
The {admc-name} chart pulls component images from `dp.apps.rancher.io`, which
is a private registry. <<step-pull-secret,Step 5>> creates the image pull
secret and <<step-install,Step 7>> passes it to the chart during installation.
